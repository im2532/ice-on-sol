...
export default function TradesTable({
  ...
  const sorted = useMemo(
    () => trades.slice().sort((a, b) => b.ts - a.ts),
    [trades],
  );
  const sortedPayouts = useMemo(
    () => payouts.slice().sort((a, b) => b.ts - a.ts),
    [payouts],
  );
      ...
      <div className="overflow-x-auto scrollbar-thin">
            ...
            <table className="mono w-full min-w-[640px] border-collapse">
              ...
              <tbody>
                {sorted.slice(0, 25).map((t) => (
                  <tr key={t.sig}>
                    <td className="td py-2.5 text-[13px] text-muted">
                      {timeAgo(t.ts * 1000)}
                    </td>
                    <td
                      className={`td py-2.5 text-[13px] ${t.side === "buy" ? "text-positive" : "text-negative"}`}
                    >
                      {t.side === "buy" ? "Buy" : "Sell"}
                    </td>
                    <td className="td py-2.5 text-right text-[13px]">
                      {fmtAmount(t.baseAmount)}
                    </td>
                    <td className="td py-2.5 text-right text-[13px]">
                      {fmtAmount(t.quoteAmount)}
                    </td>
                    <td className="td py-2.5 text-right text-[13px] text-muted">
                      {usd(t.baseAmount * t.priceUsd)}
                    </td>
                    <td className="td py-2.5 text-right text-[13px] text-muted">
        ...
        {tab === "payouts" &&
          (sortedPayouts.length === 0 ? (
            <Empty>
              No holder payout has settled on this market yet. The first lands
              within fifteen minutes of the first trade.
            </Empty>
          ) : (
              ...
              <tbody>
                {sortedPayouts.map((p) => (
                  <tr key={p.id}>
                    ...
                    <td className="td py-2.5 text-[13px]">
...
function Empty({ children }: { children: React.ReactNode }) {
  return (
    <p className="px-5 py-10 text-center text-sm text-muted">{children}</p>
  );
}
