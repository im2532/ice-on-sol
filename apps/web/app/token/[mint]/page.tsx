...
import { curveLabel, signed } from "@/lib/visual";
...
const RANGE_SEC: Record<Range, number> = {
  "1h": 3600,
  "1d": 86_400,
  "7d": 604_800,
  "30d": 2_592_000,
...
export default function TokenPage() {
  ...
  const { data: payouts } = useQuery({
    queryKey: ["recent-payouts", 12],
    queryFn: () => fetchRecentPayouts(12),
  });
  ...
  return (
    <div className="agentic-page container-x pb-16 pt-7 md:pt-9">
        ...
        <div className="token-identity flex items-center gap-4 sm:gap-[18px]">
          ...
          <div className="flex min-w-0 flex-col gap-1.5">
            <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
              ...
              <span className="mono text-sm text-positive sm:text-base">
                {market.ticker}
              </span>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <Link
                href={`/commodities/${market.commoditySymbol}`}
                className="chip tap gap-1.5 pl-2"
              >
      ...
      {(halted || closed) && (
        <p
          role="alert"
          className={`chip ${halted ? "chip-warn" : ""} mt-4 h-auto py-2`}
      ...
      <div className="detail-grid mt-6">
        <div className="flex flex-col gap-5">
          ...
          <div className="glass grid grid-cols-2 overflow-hidden md:grid-cols-4">
            ...
            <StatCell
              ...
              sub={
                commodity
                  ? `≈ ${usd(market.holderEarningsCoin * commodity.priceUsd, { decimals: 0 })} · all time`
                  : "all time"
              }
          ...
          >
            <div className="flex flex-wrap items-center justify-between gap-2">
              <SegmentedControl
                label="Chart denomination"
                value={denom}
                options={[
                  { value: "usd", label: "Value in USD" },
                  {
                    value: "coin",
                    label: `Price in ${market.commoditySymbol}`,
                  },
              ...
              <SegmentedControl
                label="Chart range"
                value={range}
                options={(["1h", "1d", "7d", "30d"] as Range[]).map(
        ...
        <div className="flex flex-col gap-5 lg:sticky lg:top-6">
          ...
          <section
            className="glass flex flex-col gap-3 p-4 sm:px-5"
          ...
          >
            ...
            <p className="text-[13px] leading-relaxed text-muted">
              Hold {market.ticker} and{" "}
              {pct(holderPct, { decimals: 2, showSign: false })} of every trade
              comes back to you in {market.commoditySymbol}, every fifteen
              minutes. Nothing to stake, nothing to claim.
            </p>
          </section>

          <section
            className="glass flex flex-col gap-2 p-4 sm:px-5"
          ...
          >
            ...
            <div className="flex items-center justify-between gap-2">
              <Link
                href={`/commodities/${market.commoditySymbol}`}
                className="flex min-w-0 items-center gap-2.5 hover:text-lavender"
              >
              ...
              <span className="mono shrink-0 text-sm">
                ...
                {commodity && (
                  <span className="text-muted"> / {commodity.unitShort}</span>
                )}
              </span>
            </div>
            <p className="text-xs leading-relaxed text-muted">
              Tracks a live oracle price
              {commodity
                ? `, last published ${timeAgo(Date.now() - commodity.lastPublishedAgoSec * 1000)}`
                : ""}
              . Redeemable for USDC against the protocol reserve, never for a
              physical asset.
            </p>
          </section>
        </div>
...
function StatCell({
  ...
  return (
    ...
    >
      ...
      <span
        className={`mono text-[17px] font-semibold sm:text-[18px] ${valueClass}`}
      >
        {value}
      </span>
      {sub && (
