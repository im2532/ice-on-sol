...
// Hermes price endpoints require a Pyth API key (Authorization: Bearer) since 2026-08-26; feed lookups do not.
const HERMES_HEADERS: Record<string, string> = process.env.PYTH_API_KEY ? { Authorization: `Bearer ${process.env.PYTH_API_KEY}` } : {};
...
async function main(): Promise<void> {
  ...
  // ---- commodities ----------------------------------------------------------------------------
  // Optional SEED_SYMBOLS=GLD,SLV limits a run to those symbols (budget-limited devnet seeding; re-run
  // without it later — already-seeded commodities are skipped).
  const only = new Set((process.env.SEED_SYMBOLS ?? "").split(",").map((s) => s.trim()).filter(Boolean));
  const targets = byPhase("mvp")
    .filter((c) => c.oracle.kind !== OracleKind.Composite) // index coins: create-index-coin.ts
    .filter((c) => only.size === 0 || only.has(c.symbol));
  console.log(`Seeding ${targets.length} MVP commodities on ${cluster}${only.size ? ` (SEED_SYMBOLS=${[...only].join(",")})` : ""}...`);
  const feedIds = await resolveFeedIds(targets);
  const prices = await resolveSeedPrices(targets, feedIds);

  const targetsBySymbol = new Map(targets.map((c) => [c.symbol, c] as const));
  ...
  // The SQL covers every commodity in the merged file (not just this run) so it can be re-applied whole.
  const allSql = ["-- generated by scripts/seed-commodities.ts", "begin;"];
  for (const e of merged.commodities) {
    const c = targetsBySymbol.get(e.symbol) ?? byPhase("mvp").find((x) => x.symbol === e.symbol);
    if (c) allSql.push(commoditySql(c, e));
  }
  allSql.push("commit;");
  writeFileSync(path.join(outDir, `${cluster}.sql`), allSql.join("\n") + "\n");
